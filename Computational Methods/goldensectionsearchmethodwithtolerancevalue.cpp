...
float goldensectionminsearch(node* head,float a,float b,float epsilon)
{
    float diff,x1,x2;
    node* list;
    node* ptr;
...
float goldensectionmaxsearch(node* head,float a,float b,float epsilon)
{
    float diff,x1,x2;
    node* list;
    node* ptr;
...
int main()
{
    float epsilon;
    int a=1,b=1;
    cout<<"Enter value of tolerance value:-"; 
    cin>>epsilon;
    node* head;
    ...
    if(option==1)
    {
    float minima=goldensectionminsearch(head,(float)a1,(float)b1,epsilon);
    node* list;
    float minval=0.0;
    list=head;
    while(list!=NULL)
    ...
    {
    float maxima=goldensectionmaxsearch(head,(float)a1,(float)b1,epsilon);
    node* temp;
    float maxval=0.0;
    temp=head;
    while(temp!=NULL)
